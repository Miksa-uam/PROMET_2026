...
def load_med_records(conn) -> pd.DataFrame:
    df = pd.read_sql(
        f"SELECT {','.join(fetch_from_records)} FROM {input_medical_records}", conn)
    return df
...
"""
# calc_overall_followup: calculate final intervention results and follow-up data, as of the last measurement
...
# calc_fixed_timepoints: calculate outcomes at given time points
...
# calc_time_to_targets: calculate time to reaching a given weight loss target
...
# calc_genomics_timing: IF genomics testing was done, calculate its temporal relationship to a given medical record - 
# to identify when and for how long the patient may have received nutrigenetic personalization
def calc_genomics_timing(record_info: pd.Series) -> dict:
    """
    Returns genomics timing flags based on record-level dates.
    Returns all NaN if genomics_results_date is missing.
    ...
    baseline = record_info.get('baseline_date')
    final = record_info.get('final_date')

    out['no_genomics_results_date'] = int(pd.isna(g_date))

    # If genomics date is missing, return all NaN (default dict above)
    if pd.isna(g_date):
        return out

    # From here, genomics_results_date is valid
    ...
    # else: stays NaN (correct by design)
...
def build_timetoevent_table():
    ...
    for (patient_id, medical_record_id), patient_group_measurements in grouped_measurements:
        ...
        # 4) 
        genomics_timing_dict = calc_genomics_timing(record_info_with_baseline)
