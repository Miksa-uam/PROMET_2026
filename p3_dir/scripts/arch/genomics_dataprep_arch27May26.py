...
#! NEEDS TO BE UPDATED WITH NEW COLUMNS
...
def calc_fixed_timepoints(patient_record_measurements: pd.DataFrame, 
    ...
    for w in config.time_windows:
        ...
        if window_meas.empty:
            ...
            out[f"{prefix}_vat_%"], out[f"{prefix}_vat_loss_%"] = np.nan, np.nan
            ...
            out[f"{prefix}_vat_%"] = take["vat_%"]
            out[f"{prefix}_vat_loss_%"] = take["vat_%"] - baseline_vat_pct
...
def calc_genomics_timed_outcomes(patient_record_measurements: pd.DataFrame, 
    ...
    window_span = config.window_span  # Uses the config variable (e.g., 10 days)
    
    # ---------------------------------------------------------
    # PART 1: Outcomes at the exact time of the events
    # ---------------------------------------------------------
    event_dates = {
    ...
    for event_name, target_date in event_dates.items():
        missing_key = f"measurement_missing_at_genomics_{event_name}"
        ...
        if window_meas.empty:
            ...
            days_from_baseline = (meas_date - baseline_measurement_date).days
            # Prevent div/0 for same-day measurements
            days_for_rate = days_from_baseline if days_from_baseline > 0 else 1 
            
            out[f"measurement_date_at_genomics_{event_name}"] = meas_date
            out[f"weight_kg_at_genomics_{event_name}"] = take.get('weight_kg', np.nan)
            
            # Weight & Rates
            if pd.notna(baseline_weight_kg) and baseline_weight_kg > 0:
                wl_kg = baseline_weight_kg - take.get('weight_kg', np.nan)
                wl_pct = 100 * wl_kg / baseline_weight_kg
                
                out[f"wl_kg_at_genomics_{event_name}"] = wl_kg
                out[f"wl_%_at_genomics_{event_name}"] = wl_pct
                out[f"wl_rate_kg_d_at_genomics_{event_name}"] = wl_kg / days_for_rate
                out[f"wl_rate_%_d_at_genomics_{event_name}"] = wl_pct / days_for_rate
                
            # Body Composition (Graceful handling of missing cols/values)
            metrics_map = {
                'bmi': 'baseline_bmi',
                'fat_%': 'baseline_fat_%',
                'muscle_%': 'baseline_muscle_%',
            ...
            for metric, base_col in metrics_map.items():
                val = take.get(metric, np.nan)
                base_val = baseline_row_info.get(base_col, np.nan)
                
                out[f"{metric}_at_genomics_{event_name}"] = val
                if pd.notna(val) and pd.notna(base_val):
                    # For BMI and Fat/VAT, we usually talk about reduction/loss
                    diff = base_val - val if metric != 'muscle_%' else val - base_val
                    suffix = 'change_%' if metric == 'muscle_%' else ('reduction' if metric == 'bmi' else 'loss_%')
                    out[f"{metric.split('_')[0]}_{suffix}_at_genomics_{event_name}"] = diff

    # ---------------------------------------------------------
    # PART 2: Post-Personalization Fixed Timepoints (Anchored to Results)
    # ---------------------------------------------------------
    results_date = event_dates['results']
    weight_at_results = out.get('weight_kg_at_genomics_results', np.nan)
    
    if pd.notna(results_date) and pd.notna(weight_at_results):
        # Loop through config.time_windows (or a dedicated post_genomics_windows array)
        for w in config.time_windows: 
            ...
            if window_meas.empty:
                ...
                meas_date = take.measurement_date
                weight_now = take.get('weight_kg', np.nan)
                
                days_from_results = (meas_date - results_date).days
                days_from_results = days_from_results if days_from_results > 0 else 1
                
                days_from_baseline = (meas_date - baseline_measurement_date).days
                days_from_baseline = days_from_baseline if days_from_baseline > 0 else 1
                
                out[f"{prefix}_measurement_date"] = meas_date
                out[f"{prefix}_weight_kg"] = weight_now
                
                if pd.notna(weight_now):
                    # Option B: Relative to Genomics Results (The Intervention view)
                    wl_from_results = weight_at_results - weight_now
                    wl_pct_results = 100 * wl_from_results / weight_at_results if weight_at_results > 0 else 0
                    
                    out[f"{prefix}_wl_kg_from_results"] = wl_from_results
                    out[f"{prefix}_wl_%_from_results"] = wl_pct_results
                    out[f"{prefix}_wl_rate_kg_d_post"] = wl_from_results / days_from_results
                    out[f"{prefix}_wl_rate_%_d_post"] = wl_pct_results / days_from_results

                    # Option A: Relative to Initial Baseline (The Total Treatment view)
                    if pd.notna(baseline_weight_kg) and baseline_weight_kg > 0:
                        wl_from_base = baseline_weight_kg - weight_now
                        wl_pct_base = 100 * wl_from_base / baseline_weight_kg
                        
                        out[f"{prefix}_wl_kg_from_baseline"] = wl_from_base
                        out[f"{prefix}_wl_%_from_baseline"] = wl_pct_base
                        out[f"{prefix}_wl_rate_kg_d_overall"] = wl_from_base / days_from_baseline
                        out[f"{prefix}_wl_rate_%_d_overall"] = wl_pct_base / days_from_baseline
